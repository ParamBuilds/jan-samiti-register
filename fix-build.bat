@echo off
echo 🚀 Starting complete automated build fixer...
...
powershell -Command ^
"Select-String 'Rollup failed to resolve import' build-errors.txt | ForEach-Object { ($_ -split '\"')[1] } | ForEach-Object { ^
    Write-Host ('Installing: ' + $_); ^
    try { npm install $_ --save } catch { npm install $_ --save-dev } ^
}"
